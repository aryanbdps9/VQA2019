...
from embedding import embedding_mat, word_to_index_map, get_c2i
...
exp_tb_path_base = "run/exp3/"
exp_tb_path=""
writer = None
save_path = "weights3/train.pt"
save_path2 = "weights3/train_100.pt"
...
def train2():
	num_words = len(word_to_index_map)
	...
	words = list(word_to_index_map.keys())[:word_lim]
	model = Model(rnn_layers=3)
	optimizer = optim.Adam(model.parameters(), lr=lr, amsgrad=True)
	itr = 0
	test_word = 'apple'
	test_embedd = torch.tensor(
		embedding_mat[word_to_index_map[test_word], :]).view(1, 1, -1).to(device)
		...
		for bi in range(batch_size):
			...
			while(True):
				...
				if valid_word(sel):
					break
			embedd = torch.tensor(
				embedding_mat[word_to_index_map[sel], :]).view(1, -1).to(device)
			sel = sel + "\0"
...
# 	make_exp_path(lr, lossParam, word_lim)
# 	num_words = len(word_to_index_map)
# 	words = list(word_to_index_map.keys())
# 	model = Model()
# 	optimizer = optim.Adam(model.parameters(), lr=lr, amsgrad=True)
# 	itr = 0
# 	test_word = 'apple'
# 	test_embedd = torch.tensor(embedding_mat[word_to_index_map[test_word], :]).view(1, 1, -1)
# 	if os.path.isfile(save_path):
# 		ckpt = torch.load(save_path)
# 		model.load_state_dict(ckpt['model_params'])
# 		optimizer.load_state_dict(ckpt['optim_params'])
...
# 				if valid_word(sel):
# 					break			
# 			embedd = torch.tensor(embedding_mat[word_to_index_map[sel], :]).view(1, 1, -1)
# 			sel = "\b" + sel + "\0"
